...
describe('The base repository', function () {
  ...
  it('should add an entity while defining its identifier', function () {
    ...
    return add.then(function (addedEntity) {
      addedEntity.id.should.match(constants.UUID_REGEX);
      addedEntity.name.should.equal('entity');
      database.collections.collection.should.deep.equal([addedEntity]);
    });
  });

  it('should add an entity without altering the argument', function () {
    var arg = {name: 'entity'};

    var add = repository.add(arg);

    return add.then(function () {
      arg.should.deep.equal({name: 'entity'});
    });
  });
