...
describe('The event bus', function () {
  ...
  it('should accept broadcast interception', function () {
    var interceptions = [];
    bus.addBroadcastInterceptor(function (type, data) {
      interceptions.push({type: type, data: data});
    });

    bus.broadcast('MyEvent', {the: 'data'});

    interceptions.should.deep.equal([{type: 'MyEvent', data: {the: 'data'}}]);
  });
});
